            ...
            <div class="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-5">
                <div>
                    ...
                    <p class="text-sm text-slate-500 mt-2 leading-relaxed max-w-2xl">
                        Pantau semua akun owner yang terdaftar, status akun, paket langganan, dan aktivitas login terakhir.
                    </p>
                </div>
        ...
        <div class="hidden lg:block overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-slate-50 text-slate-500">
                    <tr>
                        <th class="text-left px-6 py-4 font-black text-xs uppercase tracking-wide">Owner</th>
                        ...
                        <th class="text-left px-6 py-4 font-black text-xs uppercase tracking-wide">Subscription</th>
                        <th class="text-left px-6 py-4 font-black text-xs uppercase tracking-wide">Last Login</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    >
                        ...
                        <td class="px-6 py-5">
                            <p :class="['font-black capitalize', subscriptionTextClass(user.subscription_status)]">
                                {{ user.subscription_status || '-' }}
                            </p>
                            <p class="text-xs text-slate-500 mt-1">
                                Expired: {{ user.subscription_ends_at || '-' }}
                            </p>
                        </td>

                        <td class="px-6 py-5 text-slate-600 whitespace-nowrap">
                            {{ user.last_login_at || 'Belum login' }}
                        </td>

                        <td class="px-6 py-5">
                            <span :class="['px-3 py-1.5 rounded-full text-xs font-black capitalize', accountStatusClass(user.status)]">
                                {{ user.status || '-' }}
                            </span>
                        </td>
            ...
            >
                <div class="flex items-start gap-4">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <div class="flex flex-wrap gap-2 mt-3">
                            ...
                            <span :class="['px-3 py-1.5 rounded-full text-xs font-black capitalize', subscriptionStatusClass(user.subscription_status)]">
                                {{ user.subscription_status || '-' }}
                            </span>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-3 mt-4">
                    ...
                    <div class="col-span-2 rounded-2xl bg-slate-50 border border-slate-100 p-4">
                        <p class="text-xs text-slate-500">Subscription</p>
                        <p class="text-sm font-bold text-slate-900 mt-1">
                            {{ user.subscription_status || '-' }} • expired {{ user.subscription_ends_at || '-' }}
                        </p>
                    </div>
                </div>
            </div>
...
const accountStatusClass = (status) => {
    ...
    if (status === 'suspended') return 'bg-slate-100 text-slate-600';
...
const subscriptionTextClass = (status) => {
